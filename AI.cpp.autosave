...
void AI::MakeMove(Field* fuf)
{
    ...
    cstate st;
    bool l = false, r = false, u = false, d = false, nf = false;
    switch (this->level)
    {
    case (1) :						//*** Hard level. ***
            ...
            else if (st == fuf->water)
            {
                ...
                if (!hDir)
                {
                    if (fuf->posx>lhX && lhX == 0)
                        hDir = true;
                    if (fuf->posx<lhX && lhX == (fuf->Width()-1))
                        hDir = true;
                }
                else
                {
                    if (fuf->posy>lhY && lhY == 0)
                        hDir = false;
                    if (fuf->posy<lhY && lhY == (fuf->Height()-1))
                        hDir = false;
                }
            }
        }
